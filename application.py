...
def buy():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            ...
            return apology("please provide a full number > 0")
...
def register():
    ...
    # User reaches route via POST (when form is submitted)
    if request.method == "POST":

        # Ensure user adds username
        if not request.form.get("username"):
...
def sell():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            ...
            return apology("provide a positive full number of shares")
        else:
            shares = int(shares)
        ...
        for stock in stocks:
            if stock["symbol"] == symbol:
                if stock["total_shares"] < shares:
                    ...
                    quote = lookup(symbol)
                    if quote is None:
                        return apology("symbol ot fouund")
                    price = quote["price"]
